...
type UUCreateOrderRequest struct {
	...
	CountyName      string `json:"countyName,optional"`      // 订单所在县级地名称
	PushType        int64  `json:"pushType"`                 // 推送方式（0 正常订单，1测试订单）
	SubscribeType   int64  `json:"subscribeType"`            // 预约类型 0 实时单 1 预约取件时间 2 预约送达时间
	SubscribeTime   int64  `json:"subscribeTime,optional"`   // 预约时间时间戳（秒）
	GoodsWeight     int64  `json:"goodsWeight,optional"`     // 物品重量单位：KG
	OriginalPrice   string `json:"originalPrice"`            // 商品价格 原价(元)
	Price           string `json:"price"`                    // 商品价格 实际支付金额(元)
